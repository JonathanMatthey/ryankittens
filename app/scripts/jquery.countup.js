...
(function($) {
    ...
    var methods = {
        ...
        init: function(options) {
            if (options) {
                var newOptions = $.extend({}, _defaults, options);
            }
            ...
            _timeDifference = methods.difference(_startDate, _endDate);

            _timeDifference = methods.formatted(_timeDifference, newOptions);
            setInterval(function() {
                methods.update(_timeDifference, newOptions);
            }, 999);
        },
        ...
        print: function(time, newOptions) {
            ...
            if (newOptions['write_zero'] == 'true') {
                $(newOptions['div_id'])
                    .html(year + month + day + hour + minute + seconds);
            } else {
                $(newOptions['div_id'])
                    .html(null);
                var zero = false;
                ...
                if ((time.seconds > 0) || zero) $(newOptions['div_id'])
                    .append(seconds);
            }
        },
    ...
    $.fn.countUp = function(method) {
        _defaults['div_id'] = "#" + $(this)
            .attr('id');
        _defaults['class_prefix'] = $(this)
            .attr('id');
